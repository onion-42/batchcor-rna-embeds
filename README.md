...
│
└── visualizations/                 ← visualisation notebooks + ancillary PNGs
    ├── visualizations_cae_correction.ipynb       ← cAE quality notebook
    ├── visualizations_SOTA_analytics.ipynb       ← single-source-of-truth SOTA suite
    │                                                (~30 inline plots — every
    │                                                 figure renders in the notebook,
    │                                                 no PNG/PDF side-files emitted)
    ├── batch_correction_metrics.png              ← cAE quality (auto, by run_cae_correction)
    ├── per_cohort_silhouette.png                 ← cAE quality (auto)
    ├── TRAIN_*.png                               ← cAE pre/post diagnostics (auto)
    └── v4_*.png                                  ← v4_definitive_pipeline figures (auto)
```
