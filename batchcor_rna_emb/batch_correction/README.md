...
**Источник:** референс-нотбук `references/harmony_backtrack.ipynb` (он же скопирован в `.claude/harmony_backtrack.ipynb`); рефакторенная версия — `batchcor_rna_emb/batch_correction/harmony.py`.

**Аудитория плана:** студент-имплементатор, который должен понять схему end-to-end и собрать пайплайн на новых данных, переиспользуя готовые функции модуля.
